...
export default function AttendanceList() {
  ...
  async function startNfcScan() {
    if (!nfcSupported) {
      alert("NFC tidak didukung pada perangkat ini.");
      return;
    }

    if (sholatSession === 'None') {
      alert("Presensi hanya bisa dilakukan saat waktu sholat.");
      return;
    }
    ...
    try {
      ...
      ndef.onreadingerror = () => {
        console.error("Gagal membaca tag NFC.");
      };
  ...
  async function handleRegisterNfc(santriId: string, nfcId: string) {
    try {
      ...
      <div className="col-span-12 lg:col-span-12 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-col md:flex-row items-center gap-3">
            ...
            >
              <Smartphone className={cn("w-4 h-4", isNfcScanning && "animate-bounce")} />
              {!nfcSupported ? 'NFC Tidak Didukung' : (isNfcScanning ? (registeringNfcFor ? 'Tempelkan Kartu Baru...' : 'Scan Kartu...') : 'Absen via NFC')}
            </button>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setRegisteringNfcFor(santri.id);
                        startNfcScan();
                      }}
                      title="Daftarkan Kartu NFC"
                      className={cn(
                        "p-2 rounded-lg transition-colors",
                        santri.nfc_id ? "text-emerald-500 hover:bg-emerald-50" : "text-slate-300 hover:bg-slate-100"
                      )}
                    >
